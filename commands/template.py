...
class Template:
    ...
    async def templates(self, ctx, page: int=1):
        ...
        if len(ts) > 0:
            ...
            w1 = max(max(map(lambda tx: len(tx.name), ts)) + 2, len(ctx.getlang("template.info_name")))
            msg = [
                ctx.getlang("template.list_open").format(page, pages),
                "{0:<{w1}}  {1:<14}  {2}\n".format(ctx.getlang("template.info_name"),
                                                   ctx.getlang("template.info_canvas"),
                                                   ctx.getlang("template.info_coords"), w1=w1)
            ]
            ...
            msg.append(ctx.getlang("template.list_close").format(sql.get_guild_prefix(ctx.guild.id)))
            await ctx.send(''.join(msg))
        else:
            await ctx.send(ctx.getlang("template.list_no_templates"))
    ...
    async def templates_info(self, ctx, name):
        ...
        if not t:
            await ctx.send(ctx.getlang("template.name_not_found").format(name))
            return
        ...
        e = discord.Embed(title=t.name, url=canvas_url, color=13594340) \
            .set_image(url=t.url) \
            .add_field(name=ctx.getlang("template.info_canvas"), value=canvas_name, inline=True) \
            .add_field(name=ctx.getlang("template.info_coords"), value=coords, inline=True) \
            .add_field(name=ctx.getlang("template.info_size"), value=size, inline=True) \
            .add_field(name=ctx.getlang("template.info_added_by"), value=added_by, inline=True) \
            .add_field(name=ctx.getlang("template.info_date_added"), value=date_added, inline=True) \
            .add_field(name=ctx.getlang("template.info_date_modified"), value=date_modified, inline=True)
    ...
    async def templates_remove(self, ctx, name):
        t = sql.get_template_by_name(ctx.guild.id, name)
        if not t:
            await ctx.send(ctx.getlang("template.no_template_named").format(name))
            return
        if t.owner_id != ctx.author.id and not utils.is_template_admin(ctx) and not utils.is_admin(ctx):
            await ctx.send(ctx.getlang("template.not_owner"))
            return
        sql.drop_template(t.gid, t.name)
        await ctx.send(ctx.getlang("template.remove").format(name))

    @staticmethod
    async def add_template(ctx, canvas, name, x, y, url):
        if len(name) > cfg.max_template_name_length:
            await ctx.send(ctx.getlang("template.name_too_long").format(cfg.max_template_name_length))
            return
        if sql.count_templates(ctx.guild.id) >= cfg.max_templates_per_guild:
            await ctx.send(ctx.getlang("template.max_templates"))
            return
        ...
        if chk is not None:
            ...
            await ctx.send(ctx.getlang("template.updated").format(name))
            return
        elif await Template.check_for_duplicates_by_md5(ctx, t) is False:
            return
        sql.add_template(t)
        await ctx.send(ctx.getlang("template.added").format(name))
    ...
    async def build_template(ctx, name, x, y, url, canvas):
        try:
            with await utils.get_template(url) as data:
                ...
                if not quantized:
                    if not await utils.yes_no(ctx, ctx.getlang("template.not_quantized")):
                        return
    ...
    async def check_for_duplicates_by_md5(ctx, template):
        ...
        if len(dups) > 0:
            msg = [ctx.getlang("template.duplicate_list_open")]
            ...
            msg.append(ctx.getlang("template.duplicate_list_close"))
    ...
    async def check_for_duplicate_by_name(ctx, template):
        ...
        if dup:
            if template.owner_id != ctx.author.id and not utils.is_admin(ctx):
                await ctx.send(ctx.getlang("template.name_exists_no_permission"))
                return False
            print(dup.x)
            q = ctx.getlang("template.name_exists_ask_replace")\
