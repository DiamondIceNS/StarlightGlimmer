...
def update_guild(gid, name=None, prefix=None, alert_channel=None, emojishare=None, autoscan=None, default_canvas=None,
                 language=None, template_admin_role_id=None, template_adder_role_id=None, bot_admin_role_id=None):
    if name is not None:
...
def clear_bot_admin_role(gid):
    c.execute("UPDATE guilds SET bot_admin_role=NULL WHERE id=?", (gid,))
    conn.commit()
